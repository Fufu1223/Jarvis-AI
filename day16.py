...
def get_strategic_history(full_history: List[Dict[str, Any]], limit: int = 5) -> List[Dict[str, Any]]:
    """
    从完整的对话历史中，根据策略截取最近的 N 条记录。
    如果历史记录不足 limit 条，则返回全部。
    
    Args:
        full_history: 包含所有对话的列表
        limit: 限制发送给 API 的最大消息数量
        
    Returns:
        List[Dict[str, Any]]: 用于发送给 API 的精简列表
    """
    # 这里的 -limit 是核心：取倒数 limit 条
